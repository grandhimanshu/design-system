...
describe('Stepper component keyboard accessibility', () => {
  ...
  it('steps are keyboard-focusable when not disabled', () => {
    const active = 1;
    const completed = 0;
    const { getAllByTestId } = render(
    ...
    const stepNodes = getAllByTestId('DesignSystem-Step');
    // Steps 0, 1, 2 are enabled (active or completed); step 3 is disabled
    expect(stepNodes[0]).toHaveAttribute('tabIndex', '0');
    expect(stepNodes[1]).toHaveAttribute('tabIndex', '0');
